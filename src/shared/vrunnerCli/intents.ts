 ...
 * из неё (2 команды), а в 3.x выполняется одной командой.
 *
 ...
 * (в 3.x — строго перед позиционными аргументами, а `--settings` у команд
 * без этой опции — перед группой).
 */


/** Сквозные опции команды (аргументы, валидные в обоих CLI): пары «опция, значение». */
...
export type VRunnerIntent =
	...
	/** Создать файл поставки конфигурации (.cf). */
	 ...
	 * в 3.x — одна команда `cfe decompile --cfe-file`: в базе из строки
	 * подключения, а без строки во временной.
	 */
