...
Оба живут в группе **Автоматизация** дерева «[1С: Инструменты](tools.md)»: **Редактор пайплайнов**, сохранённые цепочки, **Редактор хуков** и команды с заданными хуками. Клик по цепочке открывает её в редакторе, запуск - кнопкой ▶ в строке.
...
Блоки выполняются без терминала: в журнал расширения пишется отчёт по шагам, у упавшего - последняя строка вывода. Запуск Предприятия и Конфигуратора шагом цепочки идёт так же, а результат шага означает, удалось ли стартовать. [Хуки](#хуки-команд) `pre`/`post`/`onError` отрабатывают в обоих случаях.
...
Хуки настраиваются в файле `.1cpt/hooks.json` в корне проекта, без привязки к [профилю запуска](launch-profiles.md). В отличие от пайплайна, хук не запускают: он срабатывает сам, когда выполняется команда.
...
Файл создаётся сам при первом открытии редактора. Он также входит в [служебные файлы](service-files.md): раздел **Служебные файлы** → **1cpt: hooks**, либо палитра «1С: Служебные файлы: Создать…».
