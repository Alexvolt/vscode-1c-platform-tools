---
layout: home

hero:
  name: '1C: Platform Tools'
  text: Инструменты разработки 1С в Visual Studio Code
  tagline: Панели проектов и метаданных, отладка, тестирование, автоматизация и MCP — всё в одном расширении.
  image:
    src: /cat-hi.png
    alt: ''
  actions:
    - theme: brand
      text: Руководства по функциям
      link: /tools
    - theme: alt
      text: VS Code Marketplace
      link: https://marketplace.visualstudio.com/items?itemName=yellow-hammer.1c-platform-tools
    - theme: alt
      text: Open VSX (Cursor, Windsurf)
      link: https://open-vsx.org/extension/yellow-hammer/1c-platform-tools
    - theme: alt
      text: GitHub
      link: https://github.com/yellow-hammer/vscode-1c-platform-tools

features:
  - title: '1С: Инструменты'
    details: Дерево команд vanessa-runner — информационная база, конфигурация, расширения, поставка, запуск и избранное.
    link: /tools
  - title: Проекты и артефакты
    details: Поиск проектов, избранное и теги, быстрое переключение; сборка и разбор cf, cfe, epf и erf.
    link: /projects
  - title: Метаданные и ER-диаграммы
    details: Дерево и свойства объектов, состав, формы и схемы компоновки, поддержка поставщика; конструктор схем связей с экспортом.
    link: /metadata
  - title: Отладка 1С
    details: Точки останова с условиями и логированием, переменные и вычисление выражений, замер производительности.
    link: /debug
  - title: Тестирование
    details: Панель тестирования VS Code — Vanessa, xUnit, YAxUnit, OneScript и 1bdd со статусами и переходом к падению.
    link: /testing
  - title: Автоматизация
    details: Пайплайны с визуальным редактором графа шагов и хуки команд до, после и при ошибке.
    link: /automation
  - title: Формат EDT
    details: Проекты 1С:EDT наравне с выгрузкой конфигуратора, те же панели, команды и тесты.
    link: /edt
  - title: Администрирование
    details: Информационные базы платформы, запуск Предприятия и Конфигуратора, консоль кластера через rac.
    link: /admin
  - title: AI-агенты
    details: Навыки для агентов и файл-триггер — агент узнаёт команды расширения и запускает их.
    link: /ai-mcp
  - title: MCP-сервер
    details: Агент вызывает команды расширения через MCP и получает результат — тесты, сборка, загрузка, окружение.
    link: /mcp/
  - title: Docker и ibcmd
    details: Выполнение команд в контейнере без локальной платформы 1С, включая GitHub Codespaces.
    link: /docker
  - title: Внешние компоненты
    details: 'Отладчик, дерево метаданных, JRE, OVM и Allure: загрузка, свои сборки, работа без доступа к GitHub.'
    link: /components
---
