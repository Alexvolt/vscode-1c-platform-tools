...
Раннер OneScript выбирается настройкой `test.onescriptRunner`: по умолчанию `auto` — OneUnit, если он установлен в проекте или объявлен в `packagedef`, иначе 1testrunner. Путь к раннеру можно задать явно настройкой `test.path.onescriptRunner`.
...
Каталоги тестовых расширений и обработок настройками не задаются: расширение находит их само, а
тестовыми их делает каталог тестов в пути, см. [Раскладка проекта](tools.md#раскладка-проекта).
Имя каталога задаёт `test.directoryName`, по умолчанию `tests`. Скриптовые тесты `*.os` лежат в
нём же, другой каталог для них задаёт `test.path.onescriptTests`.
...
Если конфиги не настроены, используются временные настройки с отчётом в `test.path.reports`.

## Настройки

| Настройка                                           | По умолчанию                   | Описание                                          |
|-----------------------------------------------------|--------------------------------|---------------------------------------------------|
| `test.panelEnabled`                              | `true`                         | Интеграция с панелью тестирования                 |
| `test.path.features`                             | `features`                     | Сценарии Gherkin                                  |
...
В дереве метаданных тестовые расширения видны
...
Все четыре показывают выбор расширений с флажками: можно подключить только YAxUnit, только своё
расширение с тестами или оба сразу. Выбор запоминается для проекта отдельно от выбора расширений
решения. Чтобы окно выбора не показывалось, перечислите нужные расширения в
`1c-platform-tools.test.cfe.selected`. Агент передаёт выбор параметром `extensions`.
...
Панель тестирования ищет тесты YAxUnit и в расширениях решения, и в тестовых:
...
Команда **Мутационное тестирование** в группе «Тестирование» проверяет тесты OneUnit через [mutatos](https://github.com/nixel2007/mutatos): он вносит в исходники по одной правке и смотрит, заметят ли её тесты. Нужны mutatos в зависимостях проекта и OneScript 2.x. Проекты на 1testrunner mutatos не прогоняет.

Та же команда есть в контекстном меню файла и теста OneScript в панели тестирования. Тогда мутанты проверяются только выбранными тестами.

После прогона строка состояния показывает индекс мутаций и число выживших мутантов, а сами выжившие попадают в Problems на место правки, которую тесты не заметили. Отчёт открывается командой **Открыть отчёт мутаций**.
...
- Источник — jUnit-отчёт прогона, путь к нему берётся из **активного профиля запуска**.
