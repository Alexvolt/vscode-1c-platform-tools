<img src="../resources/brand/cat-hi.png" alt="" width="72" align="right">

# 1С: Проекты

> <img src="../resources/brand/cat-hi.png" alt="" width="26" align="left"> Проект переключается из статусной строки, не выходя из окна.

Панель **1С: Проекты** в activity bar помогает быстро переключаться между рабочими папками с проектами 1С. Доступна даже без открытого проекта.

Первый вид панели, **Рабочая область**, показывает проекты открытого окна, см. [Проекты в рабочей области](tools.md#проекты-в-рабочей-области).

![Панель «1С: Проекты»](../walkthrough/images/step6.png)

## Поиск проектов

Вкладка **Все проекты** сканирует указанные папки и ищет каталоги с `packagedef`. Папки поиска задаются настройкой:

```json
{
  "1c-platform-tools.projects.path.baseFolders": [
    "~/projects",
    "D:/work/1c"
  ]
}
```

Если `baseFolders` пуст, вкладка «Все проекты» ничего не найдёт — избранные проекты при этом можно добавлять вручную из открытой рабочей папки. Лишние подпапки исключаются через `projects.ignorePatterns` (например `node_modules`, `out`), глубина сканирования — `projects.maxDepthRecursion`.

## Избранное и теги

- **Избранное** хранит выбранные проекты; отображение — списком или по тегам.
- Теги (например «Личное», «Работа») задаются настройкой `projects.tags` и назначаются проектам из контекстного меню.

## Быстрое переключение

- Статусная строка показывает текущий проект, а в окне без проекта 1С открытую папку. Клик открывает список: сначала проекты этого окна, выбор делает проект [текущим](tools.md#текущий-проект), затем Избранное и Все проекты для открытия.
- Команда **1С: Проекты → Открыть список** — из палитры или по `Ctrl+Alt+P` / `Cmd+Alt+P`.
- Поведение открытия (новое окно, подтверждение, фильтр по полному пути) настраивается ключами `1c-platform-tools.projects.*`.

## Синхронизация

`projects.projectsLocation` — папка для хранения `projects.json` (избранное и теги); укажите синхронизируемый каталог, чтобы переносить список между машинами. `projects.cacheBetweenSessions` ускоряет открытие панели за счёт кэша результатов сканирования.
