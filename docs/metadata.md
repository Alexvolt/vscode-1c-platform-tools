...
- показывает дерево метаданных конфигурации, расширений и внешних объектов по раскладке проекта; тестовые расширения и обработки идут наравне с остальными;
- держит дерево и граф связей в хранилище рабочей области VS Code и перечитывает их у md-sparrow только после изменения исходного кода; свойства объектов, словари значений и состав подсистем запоминаются на время сеанса и перечитываются, когда меняются сами файлы;
- расширение с версией выгрузки, которую md-sparrow не читает, остаётся в дереве узлом без состава: под ним только пометка о версии, из действий «Открыть XML» и «Собрать»;
...
2. При настройках по умолчанию расширение скачает portable JRE 21 и `md-sparrow-*-all.jar` из последнего стабильного релиза `yellow-hammer/md-sparrow`.

Настраивать для этого нечего. Свои сборки, автозагрузка, обновление и работа без доступа к GitHub описаны в разделе [Внешние компоненты](components.md).
...
У реквизита, измерения и ресурса рядом с именем, синонимом и типом идут свойства использования: подсказка, проверка заполнения, индексирование, полнотекстовый поиск, история данных, быстрый выбор, создание при вводе, история выбора и форма выбора. Параметры выбора и их связи показываются строкой: значение параметра платформа хранит типизированным, и правка его текстом потеряла бы тип. Показываются те, что у этого вида узла есть в его версии формата, а варианты значений приходят от md-sparrow вместе с русскими подписями.
